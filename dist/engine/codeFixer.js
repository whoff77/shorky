...
async function generateSpecFix(specCode, failureContext) {
    ...
    if (!apiKey) {
        throw new Error('OPENAI_API_KEY is missing or empty. In CI, ensure the secret is configured in the repository/organization settings and explicitly mapped into the workflow/action env (e.g. `OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}`).');
    }
