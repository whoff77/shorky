...
export interface OpenHealingPrOptions {
  /** Path (relative to the repo root / cwd) of the spec file that was patched. */
  specPath: string;
 ...
 */
export async function openHealingPullRequest(options: OpenHealingPrOptions): Promise<string | null> {
  ...
  const relativeSpecPath = path.isAbsolute(specPath) ? path.relative(repoRoot, specPath) : specPath;
