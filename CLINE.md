...
- **`src/fixtures/autoHealFixture.ts`** — a custom Playwright fixture (`test.extend`) providing `autoHealPage` with `clickAndHeal()`, `assertVisual()`, `assertVisualBaseline()`. Persists healed selector mappings to `src/fixtures/healed-selectors.json` (a live cache — do not treat as static data). Also resets the worker-local token counter (`resetTokensUsedThisTest()`) at fixture setup and, at teardown, attaches the tokens consumed during the test (`SHORKY_TOKENS_ATTACHMENT_NAME`) via `testInfo.attach()` for `cloudReporter.ts` to pick up — see "LLM Token Usage Tracking" below.
...
- **`src/utils/`** — `githubPr.ts` (branch/PR management for auto-heal), `healingEngine.ts` (selector self-healing + AI vision assertions), `visual-diff.ts` (pixelmatch baseline comparison), `llmClient.ts`, `testGenerator.ts`, `tokenUsage.ts` (LLM token usage tracking — see "LLM Token Usage Tracking" below).
...
**Known gap:** `src/cli/fixTrace.ts`'s `notifyShorkyCloud()` (the separate `/api/webhook` self-heal-fix flow used by the GitHub Action) does **not** yet forward `codeFixer.ts`'s `FixResult.tokensUsed` in its webhook payload — `/api/webhook` doesn't currently accept a `tokensUsed` field. Only the Playwright reporter's `/api/v1/telemetry` path accumulates tokens today. Wiring up the webhook path is a natural follow-up once `shorky-cloud`'s `webhookPayloadSchema` gains an optional `tokensUsed` field.
